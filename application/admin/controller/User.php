	...
	public function addUser(){
		...
		if(IS_POST){
			...
			foreach ($post_users as $key => $value) {
					$post_users[$key]['username'] 		= 	$value['username'];
					$post_users[$key]['salt']			= 	rand_string(6);
					
					$post_users[$key]['password']  		= 	123456;//默认统一
					$post_users[$key]['company_name']  	= 	$value['company_name'];
					$post_users[$key]['company_address']= 	$value['company_address'];
					$post_users[$key]['contact_name']  	= 	$value['contact_name'];
					$post_users[$key]['contact_phone']  = 	$value['contact_phone'];
					$post_users[$key]['channel'] 		= 	$value['channel'];
					$post_users[$key]['sales_name']  	= 	$value['sales_name'];
					$post_users[$key]['email'] 			=  	NULL; 
				
				
			}
			$uid = $model->registerUser($post_users, false);

			// if (0 < $uid) {
			// 	$userinfo = array('nickname' => $data['username'], 'status' => 1, 'reg_time' => time(), 'last_login_time' => time(), 'last_login_ip' => get_client_ip(1));
			// 	/*保存信息*/
			// 	db('Member')->where(array('uid' => $uid))->update($userinfo);
			// } else {
			if(!$uid){
	...
	public function editUsers(){
		...
		if(IS_POST){
			...
			foreach ($post_users as $key => $value) {
					...
					$data['company_address']= 	$value['company_address'];
					$data['contact_name']  	= 	$value['contact_name'];
					$data['contact_phone']  = 	$value['contact_phone'];
					$data['channel'] 		= 	$value['channel'];
					$data['sales_name']  	= 	$value['sales_name'];
					$data['password']		=  	$value['password'];
					$data['salt']			= 	$value['salt'];
					if($data['password']=='123456'){
						$reuslt = $model->editUser($data, true);
					}else{
						$reuslt = $model->editUser($data, false);
					}
					continue;					
			}
			$return['status'] =1;
